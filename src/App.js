...
const FALLBACK = {
  ...
  BWMX:17.09, UNH:286.00, LULU:285.00,
...
const ANALYST_TARGETS = {
  ...
  BWMX:29.83, UNH:373.00, LULU:380.00,
  AGIX:45.00, QTUM:180.00, BAI:60.00, XBI:160.00, UFO:75.00,
};

const EARNINGS = {
  RCAT:"Aug 2026", ACHR:"Aug 2026", ERAS:"Aug 2026", SMR:"Aug 2026",
  SOUN:"Aug 2026", RGTI:"Aug 2026", CAN:"Aug 2026", MU:"Jun 2026",
  IONQ:"Aug 2026", RZLV:"Aug 2026", EONR:"Aug 2026",
  MSFT:"Jul 2026", NVTS:"Aug 2026", PGY:"Aug 2026", TSM:"Jul 2026",
  PLTR:"Aug 2026", SOFI:"Jul 2026",
  BWMX:"Aug 2026", UNH:"Jul 2026", LULU:"Sep 2026",
};

async function fetchStock(ticker) {
  try {
    var res = await fetch("/api/stock?ticker=" + ticker);
...
function calcFib(price, closes) {
  var hi = Math.max.apply(null, closes.concat([price]));
  var lo = Math.min.apply(null, closes.concat([price]));
  var r = hi - lo || price * 0.3;
...
function Sparkline(props) {
  ...
  var trend = last >= closes[0] ? "#00e676" : "#ff5252";
  return (
...
function getExitSignal(price, buyPrice, rsi, fib, target) {
  ...
  if (pnl <= -15) {
    ...
    rec = "Ned " + Math.abs(pnl).toFixed(1) + "% under indkobspris. Overvej stop-loss.";
  } else if (rsi && rsi >= 72 && pnl > 10) {
    exitAction="SAELG NU"; exitColor="#ff5252";
    rec = "RSI " + rsi + " overkoebt og du er +" + pnl.toFixed(1) + "%. Tag gevinst.";
  } else if (nearHigh && pnl > 20) {
    exitAction="SAELG NU"; exitColor="#ff5252";
    rec = "Naer 60-dages top. +" + pnl.toFixed(1) + "%. Tag profit.";
  } else if (target && price >= target * 0.95) {
    exitAction="MAL NAET"; exitColor="#00e676";
    rec = "Analyst maal $" + fmt(target) + " naesten naaet. +" + pnl.toFixed(1) + "%.";
  } else if (pnl >= 50) {
    exitAction="TAG PROFIT"; exitColor="#ffd740";
    rec = "+" + pnl.toFixed(1) + "%. Overvej at saelge 50%.";
  } else if (pnl >= 25) {
    exitAction="SAET STOP"; exitColor="#ffd740";
    rec = "+" + pnl.toFixed(1) + "%. Saet trailing stop paa +10-15%.";
  } else if (pnl > 0) {
    exitAction="HOLD"; exitColor="#69f0ae";
    rec = "+" + pnl.toFixed(1) + "%. Hold mod maal $" + fmt(target) + ".";
  } else {
    exitAction="VENT"; exitColor="#ff9800";
    rec = pnl.toFixed(1) + "%. Vent paa signal vender.";
  }
...
export default function App() {
  var [stocks,    setStocks]    = useState(function() {
    return ALL_STOCKS.map(function(s) { return Object.assign({}, s, { price:null, closes:[], signal:null, live:false, change:null, changePct:null }); });
  });
  ...
  async function scanAll() {
    ...
    var results = await Promise.all(ALL_STOCKS.map(async function(s) {
      ...
      return Object.assign({}, s, {
        price:price, closes:closes, signal:signal,
  ...
  function toggleAuto() {
    ...
    else { setAutoOn(false); clearInterval(timerRef.current); }
  }

  useEffect(function() {
    if (autoOn) { timerRef.current = setInterval(scanAll, 10*60*1000); }
  ...
  stocks.forEach(function(s) {
    ...
    var sh = shares[key];
    if (sh && s.price) {
      portfolioValue += sh * s.price;
      if (s.change) portfolioDayChange += sh * s.change;
    }
  });
  ...
  function StockRow(props) {
    ...
    var sig = s.signal;
    var newsUrl = "https://finance.yahoo.com/quote/"+s.ticker+"/news/";
    return (
      <div style={{marginBottom:6}}>
        <div style={{background:isOpen?(s.color+"0e"):"rgba(255,255,255,0.025)",border:"1px solid "+(isOpen?(s.color+"55"):"rgba(255,255,255,0.07)"),borderRadius:isOpen?"10px 10px 0 0":"10px",padding:"12px 14px"}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <div style={{flex:1,minWidth:0}} onClick={function(){setOpenKey(isOpen?null:key);}} >
              <div style={{display:"flex",alignItems:"baseline",gap:8,flexWrap:"wrap"}}>
                <span style={{fontSize:14,fontWeight:900,color:s.color,fontFamily:"monospace",cursor:"pointer"}}>{s.ticker}</span>
            ...
            {s.closes && s.closes.length > 3 ? <Sparkline closes={s.closes.slice(-20)} color={s.changePct != null ? (s.changePct >= 0 ? "#00e676" : "#ff5252") : s.color} /> : null}
            {s.price ? (
              <div style={{textAlign:"right",flexShrink:0,minWidth:52}} onClick={function(){setOpenKey(isOpen?null:key);}}>
            ...
            <a href={newsUrl} target="_blank" rel="noreferrer" style={{fontSize:16,textDecoration:"none",flexShrink:0}} title="Nyheder">📰</a>
          </div>
        </div>
        {isOpen && sig && s.price ? (
          <div style={{background:"rgba(0,0,0,0.5)",border:"1px solid "+s.color+"22",borderTop:"none",borderRadius:"0 0 10px 10px",padding:14}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
              <div>
                ...
                {ANALYST_TARGETS[s.ticker] ? (
                  <div style={{marginTop:10,fontSize:10,color:"#ffd740",fontFamily:"monospace"}}>
                    Analyst maal: ${fmt(ANALYST_TARGETS[s.ticker])}
                    <span style={{color:"#555",marginLeft:6}}>
                      ({((ANALYST_TARGETS[s.ticker]-s.price)/s.price*100).toFixed(1)}% upside)
                    </span>
                  </div>
                ) : null}
              </div>
              <div>
                ...
                {[["Top",sig.fib.hi,false],["38.2%",sig.fib.f382,true],["50%",sig.fib.f500,false],["61.8%",sig.fib.f618,true],["78.6%",sig.fib.f786,false],["Bund",sig.fib.lo,false]].map(function(row) {
                  ...
                  return (
                    <div key={row[0]} style={{display:"flex",justifyContent:"space-between",padding:"2px 5px",borderRadius:4,marginBottom:3,background:(near?s.color+"22":(row[2]?"#ffd74008":"transparent")),border:(near?"1px solid "+s.color+"44":"1px solid transparent")}}>
            ...
            <div style={{display:"flex",gap:8}}>
              <a href={"https://finance.yahoo.com/quote/"+s.ticker+"/news/"} target="_blank" rel="noreferrer" style={{flex:1,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:7,padding:"7px 10px",color:"#aaa",fontSize:11,textDecoration:"none",textAlign:"center"}}>Nyheder</a>
              <a href={"https://finance.yahoo.com/calendar/earnings?symbol="+s.ticker} target="_blank" rel="noreferrer" style={{flex:1,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:7,padding:"7px 10px",color:"#aaa",fontSize:11,textDecoration:"none",textAlign:"center"}}>Earnings</a>
              <a href={"https://finance.yahoo.com/chart/"+s.ticker} target="_blank" rel="noreferrer" style={{flex:1,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:7,padding:"7px 10px",color:"#aaa",fontSize:11,textDecoration:"none",textAlign:"center"}}>Graf</a>
            </div>
          </div>
        ) : null}
      </div>
  ...
  function ExitRow(props) {
    var s = props.s;
    var key = s.ticker+"_"+s.group;
    var bp  = positions[key];
    var sh  = shares[key];
    var sig = s.signal;
    var rsi = sig ? sig.RSI : null;
    var fib = sig ? sig.fib : null;
    var target = ANALYST_TARGETS[s.ticker];
    var exit = (s.price && bp) ? getExitSignal(s.price, bp, rsi, fib, target) : null;
    var isEditing = editKey === key;
    var totalVal = (sh && s.price) ? sh * s.price : null;
        ...
        <div style={{background:(exit?exit.exitColor+"0d":"rgba(255,255,255,0.025)"),border:"1px solid "+(exit?exit.exitColor+"44":"rgba(255,255,255,0.07)"),borderRadius:12,padding:"13px 14px"}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              {exit ? (
                ...
                <div style={{fontSize:11,color:"#555",fontStyle:"italic",marginTop:2}}>
                  {bp ? "Scanner..." : "Ingen indkobspris sat"}
                </div>
              )}
            </div>
            <div style={{textAlign:"right",flexShrink:0,marginRight:4}}>
              ...
              {sh ? <div style={{fontSize:10,color:"#666",fontFamily:"monospace"}}>{sh} aktier</div> : null}
            </div>
          ...
          {(exit && !isEditing) ? (
            <div style={{marginTop:10,fontSize:12,color:"#888",lineHeight:1.7,borderTop:"1px solid rgba(255,255,255,0.05)",paddingTop:8}}>
              {exit.rec}
              {exit.target ? <span style={{color:"#ffd740",marginLeft:4}}>Maal: ${fmt(exit.target)}</span> : null}
            </div>
          ) : null}
        </div>
      </div>
    ...
    <div style={{minHeight:"100vh",background:"#020408",color:"#ccc",fontFamily:"Georgia, serif",maxWidth:720,margin:"0 auto"}}>
          ...
          {[["signals","Entry"],["exit","Exit"],["portfolio","Portfolio"]].map(function(t) {
            return (
              <button key={t[0]} onClick={function(){setActiveTab(t[0]);}}
                style={{background:(activeTab===t[0]?"rgba(255,255,255,0.08)":"transparent"),color:(activeTab===t[0]?"#fff":"#555"),border:"1px solid "+(activeTab===t[0]?"rgba(255,255,255,0.15)":"rgba(255,255,255,0.06)"),borderRadius:20,padding:"5px 14px",fontSize:11,cursor:"pointer",fontWeight:(activeTab===t[0]?700:400)}}>
                {t[1]}
              </button>
            );
          })}
      ...
      {activeTab==="signals" ? (
        <div style={{padding:"12px 14px 40px"}}>
          ...
          <SectionHeader emoji="💎" label="UNDERVALUED" color="#fb7185" count={undervalued.length} />
      ...
      {activeTab==="portfolio" ? (
        <div style={{padding:"12px 14px 40px"}}>
          <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:12,padding:16,marginBottom:16}}>
            ...
            <div style={{fontSize:14,fontWeight:700,color:(portfolioDayChange>=0?"#00e676":"#ff5252"),fontFamily:"monospace",marginTop:4}}>
              {portfolioDayChange>=0?"+":""}{fmt(portfolioDayChange)} i dag
            </div>
            {portfolioValue === 0 ? <div style={{fontSize:12,color:"#444",marginTop:8}}>Gaa til Exit fanen og tilfoej dine aktier og antal for at se portefoljens vaerdi her.</div> : null}
          </div>
          {stocks.filter(function(s){ return shares[s.ticker+"_"+s.group]; }).map(function(s) {
            var key = s.ticker+"_"+s.group;
            var sh = shares[key];
            var bp = positions[key];
            var val = sh && s.price ? sh * s.price : null;
            var pnl = (val && bp && sh) ? ((s.price - bp) / bp * 100) : null;
            var dayChg = sh && s.change ? sh * s.change : null;
            return (
              <div key={key} style={{background:"rgba(255,255,255,0.025)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:10,padding:"12px 14px",marginBottom:6}}>
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  <div style={{width:9,height:9,borderRadius:"50%",background:s.color,flexShrink:0}}/>
                  <div style={{flex:1}}>
                    <span style={{fontSize:13,fontWeight:900,color:s.color,fontFamily:"monospace"}}>{s.ticker}</span>
                    <span style={{fontSize:10,color:"#444",marginLeft:8}}>{sh} aktier</span>
                  </div>
